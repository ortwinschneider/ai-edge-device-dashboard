...
```bash
podman build --platform=linux/amd64 -t quay.io/oschneid/chatbot .
```
If you want to run the container locally, execute:

```bash
podman run -it --rm -d -p 8080:8080 --name web quay.io/oschneid/chatbot
```

Push the container image to the registry:

```bash
podman push quay.io/oschneid/chatbot
```
